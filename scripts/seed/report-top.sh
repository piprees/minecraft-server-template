...
# ./dev seed-roll banks its measurements under .seedtest/ and scores with
# score-dimensions.py — when the legacy roll-seeds.sh CSV is absent but
# seed-roll data exists, report from that instead of erroring out.
SEEDTEST_DIR="$PROJECT_ROOT/.seedtest"
if [[ ! -f "$CSV" && -f "$SEEDTEST_DIR/measurements.csv" ]]; then
  echo "Using seed-roll results from $SEEDTEST_DIR (no legacy roll-seeds.sh CSV found)."
...
if [[ ! -f "$CSV" ]]; then
  ...
  echo "Run ./dev seed-roll first to bank measurements." >&2
  exit 1
fi
