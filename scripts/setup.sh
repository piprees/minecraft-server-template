...
if [[ $HAS_OP -eq 1 ]]; then
  ...
  op_load DISCORD_CHAT_CHANNEL_ID
...
if [[ $IS_RERUN -eq 1 ]]; then
  REQUIRED_VARS=(OPS WHITELIST SERVER_PORT MEMORY RCON_PASSWORD DOMAIN
    DISCORD_BOT_TOKEN DISCORD_CHANNEL_ID DISCORD_CHAT_CHANNEL_ID DISCORD_WEBHOOK_URL)
  if [[ $IS_CLOUD -eq 1 ]]; then
