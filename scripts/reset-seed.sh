...
echo "=================================================================="
echo ""
echo " WATCH FOR A WATCHDOG KILL IN THE NEXT FEW MINUTES (TROUBLESHOOTING.md#t57)."
echo " mc-backup runs its first backup ${BACKUP_INITIAL_DELAY:-2m} after starting, and on a"
echo " fresh world that save-all lands inside the priming pass. The entity"
echo " flush can block the tick past MAX_TICK_TIME and the watchdog kills the"
echo " JVM. The world survives; the priming pass restarts."
echo ""
echo "   ./ops ssh 'docker inspect mc --format \"{{.RestartCount}}\"'"
echo ""
echo " To avoid it entirely, stop the sidecar until priming has finished:"
echo ""
echo "   ./ops ssh 'docker stop mc-backup'"
echo "   ./ops ssh 'docker start mc-backup'    # once the map shows dimensions rendering"
echo ""
echo " Or set BACKUP_INITIAL_DELAY in the GitHub production environment before"
echo " the next full deploy — the server's .env is CI-generated, so a hand-edit"
echo " there does not survive."
echo ""
