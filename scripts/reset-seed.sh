...
echo "=================================================================="
echo ""
echo " STOP BOTH SIDECARS NOW (TROUBLESHOOTING.md#t57)."
echo " deploy.sh kept mc-backup stopped for the whole deploy, so its timer"
echo " starts from the resume above: its save-all flush lands ${BACKUP_INITIAL_DELAY:-2m} from"
echo " now, and idle-tasks issues its own a minute after that before starting"
echo " Chunky. Either flush can block the tick past MAX_TICK_TIME on a world"
echo " whose every dimension was created minutes ago, and the watchdog kills"
echo " the JVM. The world survives; the boot restarts."
echo ""
echo "   ./ops ssh 'docker stop mc-backup idle-tasks'"
echo "   ./ops ssh 'docker start mc-backup idle-tasks'   # once the map shows dimensions rendering"
echo ""
