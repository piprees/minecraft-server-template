...
| `check-client-parity.py` | The client manifest against the server mod list, both directions. A mod on one side and not the other is a registry-handshake kick for every player; gates `lint.yml` |
...
| `sample-climate-grid.sh` | One dimension's six climate axes on an N x N grid over its playable square, in a single RCON round trip. `n=41` matches what the seed roller measures with |
| `score-biome-table.py` | Which listed biomes hold ground, scored from a `/customdim biome-table` dump against the committed climate samples — and what a band `offset` would change (`--sweep`). Plain output models the square world; `--disc` models `FactsEngine`'s own disc sampling, so it predicts a `facts` figure. Reads high against `facts` — see the header |
| `check-band-share.py` | How much of its own dimension each explicit biome band holds, by vanilla's nearest-hypercube lookup over the 41x41 grids in `config/custom-dimensions/grids-41/`. A diagnostic, not a gate; a small share is not a defect ([K7](TROUBLESHOOTING.md#k7)) |
