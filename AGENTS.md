...
6. **Kuma is config-driven.** `config/uptime-kuma/kuma-config.json` is authoritative; kuma-init re-syncs every deploy and resurrects monitors deleted only via the UI. `KUMA_API_KEY` is a socket.io **session token** (`kuma-token.sh --remote`), not the Prometheus API key. **Kuma has 2FA: password-only logins always fail `authIncorrectCreds`** — the password isn't wrong, it's missing a TOTP; the session token is the only headless auth path. **The token must live in the GitHub `production` secret `KUMA_API_KEY`**, not just `.env` — every full CI deploy regenerates the server `.env` from GitHub secrets, and an empty secret wipes a hand-installed token (2026-07-11: this left a deploy maintenance window open for hours). Never retry Kuma logins in a loop — it rate-limits auth and the host runs fail2ban. A stuck "Deploy in progress" window can be cleared without creds: `docker exec uptime-kuma sqlite3 /app/data/kuma.db "DELETE FROM maintenance WHERE title='Deploy in progress';"` then restart uptime-kuma.
