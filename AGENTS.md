...
12. **Don't repeatedly poll CI runs.** Check once after dispatching; if it's in progress, give the user the Actions URL and stop. A release run takes ~47 minutes: the default smoke variant is 43m of it, because the render-check matrix walks 15 dimension:seed pairs and the slowest single dimension eats ~11m. One background check with a generous timeout is fine; five manual polls is not.
