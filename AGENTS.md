...
**Git:** conventional-commit style, imperative mood (`fix:`, `feat:`, `chore:`). Small-team habits: commit straight to main (no PRs/worktrees); park unfinished work as a WIP commit, never `git stash` (the stash stack is shared across agent sessions); after any release, refresh the major tag with `git fetch origin '+refs/tags/v4:refs/tags/v4'` — releases force-move it and a stale local copy makes fetch complain forever. The local consumer server (`~/Projects/elfydd`) is shared — check nobody is mid-test before restarting its containers.
...
| seeds.DOMAIN | **Local profile only.** Page hosted by the mod (`web/SeedServer`, `SEED_VIEWER_PORT`, default 8765); nav injected by `config/nginx/nav-proxy-seeds.conf.template` | the `mc` container itself | Local only — never deployed |
| 404 page | Heredoc in `scripts/build-modpack.sh` → `modpack/dist/404.html` | `build-modpack.sh` | Pull |

**The nav bar is injected, not authored per page**: `config/nginx/nav-proxy.conf` `sub_filter`s the nav HTML + CSS into every page — **five near-identical copies**, one per `server` block, across two files. Changing the nav MARKUP means changing all five: `grep -c '<nav' config/nginx/nav-proxy.conf.template config/nginx/nav-proxy-seeds.conf.template`.
