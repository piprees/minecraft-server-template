...
13. **BlueMap is a standalone sidecar (since v2.14.0) — there is no `bluemap` RCON.**
    ...
    - Cause: the BlueMap mod was removed; the `bluemap` container renders and serves the map (watches region files, stays up during autopause). Diagnose with `docker logs bluemap`, restart with `docker restart bluemap`, force re-renders with `./ops map render`. The image is **pinned to v5.7**: newer CLI versions reject the existing config format ("Your map-configuration is outdated") — bumping the tag requires following BlueMap's changelog migrations. Verifying a Docker tag exists (versions rule below) is NOT enough; a newer version can pull fine and then reject your configs at runtime.
