...
| 3.4 | **`lint.yml`'s bundle-manifest job has a coverage gap.** It catches scripts referenced by `ops`, `ALLOWED_COMMANDS` entries, and `$SCRIPT_DIR/*.sh` inside already-manifested scripts. A script wired any other way ships silently missing while CI stays green. | `FINDINGS.md` §8.5 |
