...
- **Ignition spends nothing unless the dimension asks.** Vanilla damages a flint and steel; an eye of ender is consumed because it is PLACED into an `end_portal_frame`, which is not an ignition. `IgniterSpend.of(damageable, creative, consumesIgniter)` is the whole decision — creative pays nothing, `consumesIgniter` takes the item, damageable takes one point, everything else is untouched. `PortalIgnitionMixin.spendIgniter` only applies it, using the `ServerWorld` overload of `ItemStack.damage` where there is no player (a dispenser): the player-taking overload NPEs on null, and `decrement(1)` on a stack of one destroys the tool.
...
- **A refused ignition says why.** Every gate answers with an `IgnitionRefusal`, and `IgnitionScan.sweepDetailed` keeps the refusal that got FURTHEST — one click reaches 349 candidate cells on three axes, so the first refusal is noise. Grep `portal ignition refused`. A click on the portal's OWN frame material logs at INFO; every other click is DEBUG, because `useOnBlock` runs for every right-click in the game. `OPENING_NOT_ENCLOSED` names the block the fill ran into and its position, which is the whole answer when one frame block has been changed. Orientation (`AXIS_NOT_ALLOWED`) is checked LAST, after the geometry, or a frame that is right in every other way is blamed for whichever axis leaked first.
