 ...
 * </ol>
 *
 * <p>Every criterion is weighted 1 WITHIN its tier. That is a deliberate
 * starting point, not an oversight: a hand-fitted weight is an unverifiable
 * guess, and nothing here has labelled data yet to justify one criterion
 * outweighing another. The one weighting that is not a guess is the tier
 * split — see {@link Criterion#tier()} — because "did it do what was asked"
 * and "is it a good world" are two questions rather than one, and pooling
 * them would let whichever tier holds more criteria decide the headline.
 */
public final class Scorer {
    ...
    public static Scorecard score(SeedFacts facts, DimensionConfig def,
        ...
        java.util.EnumMap<Criterion.Tier, double[]> tierTotals =
        ...
        for (Criterion c : criteria) {
            ...
            if (r instanceof Criterion.Result.Pass p) {
                ...
                ceiling += 1.0;
                graded++;
                double[] tier = tierTotals.computeIfAbsent(c.tier(), k -> new double[2]);
                tier[0] += s.value();
                tier[1] += 1.0;
                ...
                if (!c.gate()) {
                    ceiling += 1.0;
                    tierTotals.computeIfAbsent(c.tier(), k -> new double[2])[1] += 1.0;
                }
            }
        }

        Map<Criterion.Tier, Scorecard.Tally> tiers = new java.util.EnumMap<>(Criterion.Tier.class);
        for (var e : tierTotals.entrySet()) {
            tiers.put(e.getKey(), new Scorecard.Tally(e.getValue()[0], e.getValue()[1]));
        }
        ...
        if (failedGate != null) {
            return new Scorecard(facts.dimension(), facts.seed(),
                    ...
                    achieved, ceiling, entries, tiers);
        }
        ...
        return new Scorecard(facts.dimension(), facts.seed(),
                Scorecard.Verdict.SCORED, "", achieved, ceiling, entries, tiers);
    }
