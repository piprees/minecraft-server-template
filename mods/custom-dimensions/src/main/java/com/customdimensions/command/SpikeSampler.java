...
public final class SpikeSampler {
    ...
    public static Sample sample(Rig rig, int x, int z) {
        ...
        if (rig.climateOnly()) {
            ...
            // OCEAN_FLOOR_WG, not WORLD_SURFACE_WG: this is the FLOOR, and
            // water is not floor. WORLD_SURFACE_WG's predicate is
            // "not air", so a flooded column answers the water's surface —
            // which made a void under a raised sea level report ground
            // everywhere, at exactly seaLevel, on every seed. That is the
            // same reading the ceilinged branch above already takes (it
            // scans for the first opaque block, and water is not opaque), so
            // both branches now answer one question rather than two.
            try {
                height = rig.generator().getHeight(x, z, Heightmap.Type.OCEAN_FLOOR_WG,
    ...
    /**
     * Whether this generator's default fluid is a fluid at all — the cheap
     * gate for "could a groundless column hold anything", never a promise
     * that every groundless column does. {@code aquifers_enabled} makes the
     * fluid level noise-driven per region, so a column with no floor can
     * still be air all the way down even when this answers true; only
     * {@link #groundlessHoldsFluid} tells the two apart.
     */
    public static boolean floodsVoid(ChunkGenerator generator) {
        return generator instanceof NoiseChunkGenerator noiseGen
                && !noiseGen.getSettings().value().defaultFluid().getFluidState().isEmpty();
    }

    /**
     * Whether a groundless column still holds fluid somewhere in its band —
     * the real answer {@link #floodsVoid} alone cannot give.
     *
     * <p>Only worth calling where {@link #sample} already reported this
     * column groundless in a generator {@link #floodsVoid} says could flood
     * it: it pays for a fresh {@code ChunkNoiseSampler} via
     * {@code getColumnSample}, the same cost {@link #sample}'s ceilinged
     * branch already pays for height, so calling it for every column would
     * double that cost across a mostly-void, mostly-ceilinged dimension.
     * {@code false} on a build failure — an aquifer that cannot be read is
     * not evidence of one, the same default {@code CandidateRender.waterAt}
     * uses for a column it cannot resolve.
     */
    public static boolean groundlessHoldsFluid(Rig rig, int x, int z) {
        ...
        try {
            ...
            int top = rig.heightLimit().getTopY() - 1;
            int bottom = rig.heightLimit().getBottomY();
            return ColumnScan.holdsFluid(top, bottom,
                    y -> !column.getState(y).getFluidState().isEmpty());
        } catch (Exception e) {
            return false;
        }
    }
    ...
    /**
     * Whether the block at a grounded column's surface is fluid.
     *
     * <p>Both height sources answer the first OPEN block above the floor —
     * {@code OCEAN_FLOOR_WG} by the heightmap's own convention, and
     * {@link ColumnScan#findPlayableFloorY} by returning {@code y + 1} — so
     * the state at that y is air or fluid and nothing else. One read, no scan.
     *
     * <p>This replaces {@code height <= seaLevel}, which carries the same
     * assumption {@link #groundlessHoldsFluid} exists to remove: that being
     * under the sea line means being under water. Aquifers break it both ways
     * — measured on {@code the_catalyst_maw} at sea level 63, 116 columns sit
     * dry with a floor below it and 11 sit wet with a floor above it.
     *
     * <p><b>Null when the column could not be read</b>, which is not the same
     * as dry. This answer replaces a rule that always produced one, so a
     * failure that silently read {@code false} would move a banked water
     * fraction toward zero and look like a measurement. The caller decides
     * what to do with not-knowing; {@code FactsEngine} falls back to the sea
     * line for that column.
     */
    public static Boolean surfaceHoldsFluid(Rig rig, int x, int z, int surfaceY) {
        if (!rig.ok() || rig.noiseConfig() == null) {
            return null;
        }
        if (surfaceY < rig.heightLimit().getBottomY()
                || surfaceY >= rig.heightLimit().getTopY()) {
            return null;
        }
        try {
            var column = rig.generator().getColumnSample(x, z, rig.heightLimit(), rig.noiseConfig());
            return !column.getState(surfaceY).getFluidState().isEmpty();
        } catch (Exception e) {
            return null;
        }
    }
