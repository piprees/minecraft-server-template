...
`"noiseSettings": "adventure:end_open"` is the third preset: vanilla's End with its
origin-anchored island field replaced by a plain noise, so an `end` dimension gets End
terrain shape and scattered islands without the central island and void moat at world
origin that every end dimension otherwise inherits. `minecraft:end_islands` special-cases
the centre cell inside the type, so no constant can remove it without removing every
outer island too — `scripts/gen-end-open-preset.py` maps a noise onto the range that type
was measured at instead, and `--gain` sets how much of the plane carries island.
...
- **`spacing`** maps structure SET ids to exact `{spacing, separation}` values, overriding the theme-based `structureDensity` factors for those sets.
