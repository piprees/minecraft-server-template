...
- **`biomes`** entries may be `{id, parameters}` objects with explicit multi-noise intervals (a number or `[min, max]` per axis, -2..2). The biome claims exactly that region; unset axes span everything. A biome may carry several such entries and holds each region, the way a vanilla parameter table gives one biome a hypercube per climate region — but a repeat stating nothing new (identical `parameters`, or a bare-string entry beside a banded one) places nothing and fails `scripts/check-biome-bands.py`.
