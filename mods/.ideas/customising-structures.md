...
> **Status (2026-07-23): the recommendations SHIPPED as `structureDensity`**
> (per-dimension presets + peaceful overlay + themes) — see mods/AGENTS.md
> and docs/customisation.md. **`customising-structures.csv` is a BUILD
> INPUT**: `scripts/gen-structure-presets.py` reads it to generate the
> dense/sparse override presets, and `structure-sets-extracted.csv` is
> regenerated by `scripts/extract-structure-sets.py` — do not delete or
> hand-edit either without re-running the generators. Options B/C (Sparse
> Structures / Structurify mods) were not adopted. Retained as the audit
> reference behind the CSVs.
