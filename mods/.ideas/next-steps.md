...
Spec: `further-portal-customisations.md` (researched + written 2026-07-23, code-anchored to the real seams). Tiered: FrameMatcher abstraction + block tags + orientation control are cheap; door/doorway/ end-exit shapes and per-part materials are medium; end_gateway single-block teleporters + pattern-template grammar are deep. Work the tiers in order; every shape needs its own bot recipe (§6 of the doc).
