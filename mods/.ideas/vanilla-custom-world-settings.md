...
> **Status (2026-07-23): Tiers 1–2 SHIPPED.** Tier 1: `coordinateScale`, `effects`, `infiniburn`, `monsterSpawnLightLevel`, `monsterSpawnBlockLightLimit` (in `DimensionConfig.Environment`/`DimensionTypeBuilder`). Tier 2: `checkerboard` generator type (+`checkerboardScale`), superflat custom `layers`/`flatBiome`, and `seedRoll: {skip: true}` (which did NOT previously exist anywhere — it was added to both the mod schema and the roller's `rollable()`); roller parity via `CheckerboardBiomeSampler` (grid formula mirrored, live-verified probe-for-probe). Tier 3 and the precision-placement section remain the open roadmap; the implementation sketches from the 2026-07-22 platform handoff are merged in below.
