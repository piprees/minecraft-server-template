...
- **Carpet is NOT a platform default** — it crashed a player's server via an
  unconditional mixin conflict with Supplementaries on any piston tick (see
  `docs/known-issues/carpet-supplementaries-piston-crash.md`). Add it to the
  CONSUMER overlay for the duration of a test and take it out again:
  `echo 'carpet:f2mvlGrg' >> overlay/mods-extra.txt && ./dev up`.
