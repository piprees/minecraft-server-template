...
- **Never sync-load or force-generate a chunk** from a tick path, the immersive projector, the audio pass, entity pass-through, or an aura pass. **`getWorldChunk(cx, cz, false)` WAITS** — its only instant answer is "no holder exists", so the moment a chunk carries a ticket the holder exists, the future is not done, and the three-argument call blocks until the terrain generates. Use `PortalHelper.residentChunk` / `isColumnResident`, which go through the **two-argument** `ServerChunkManager.getWorldChunk(cx, cz)`; that one cannot wait (cache scan, `getChunkHolder`, `getOrNull`, else null). Null means skip. `TickPathChunkLoadTest` walks the call graph in bytecode from every tick entry point, but its predicate flags exactly two shapes: a `net/minecraft/` call named `getChunk`, and `getWorldChunk` with descriptor `(IIZ)`. **It is a guard against those, not a proof of safety** — anything else on the graph passes whatever it does, so a new acquisition route needs reading, not a green suite.
...
- **A refused ignition names its reason.** Grep `portal ignition refused` — a click on the portal's own frame material is INFO, every other click DEBUG. Read that line before touching a portal that will not light.
- **Lighting a portal spends nothing by default.** A damageable igniter takes 1 damage, vanilla's flint-and-steel behaviour; consumption is opt-in per dimension (`portal.consumesIgniter`). `IgniterSpend` decides and the mixin applies — a decrement on a stack of one destroys the tool.
- **A portal is a frame with NOTHING inside it**, at either end. The frame plus its particles and projection is the portal, so a broken frame is not a portal — there is nothing else for it to be. Frames are written with `Block.NOTIFY_LISTENERS | Block.FORCE_STATE`; `NOTIFY_ALL` cascades into a neighbouring piston.
...
- **Both ends are zones.** A source zone carries the route out; an arrival zone carries the frame the mod built and is validated the same way. `PORTAL_TARGETS` holds the per-cell return route, and "am I standing in a portal" is a lookup in it — never a block read. Assert success by traversal, never by probing an interior.
