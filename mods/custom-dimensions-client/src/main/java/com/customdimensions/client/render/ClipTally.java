 ...
 *
 * <p>This is the only per-pose surface. The emit line is on a wall-clock
 * cadence, so {@link com.customdimensions.client.Repeated} drops it to DEBUG
 * after the first of a session and a later pose never reaches the log at INFO.
 * The camera's position and the bucket counts are here and on no line at all.
 *
...
public final class ClipTally {
    ...
    /**
     * One layer's outcome. {@code kept} of {@code total} buckets reached the
     * clip at all, and {@code quadsIn} counts only those — a bucket rejected
     * against the opening never adds to it.
     *
     * <p>{@code rejectedBy} is indexed by the opening's edges in build order:
     * low A, high B, high A, low B. A quad leaves the loop at the first edge
     * that cuts it, so a later edge reads low when an earlier one already took
     * everything, and the slots are not comparable across portals of different
     * normal axis.
     */
    public record Layer(String layer, int kept, int total, int quadsIn, int emitted,
            int[] rejectedBy) {}
    ...
    public static void layer(BlockPos aperture, String name, int kept, int total,
            int quadsIn, int emitted, int[] rejectedBy) {
        Portal portal = BY_APERTURE.get(aperture);
        if (portal == null) {
            return;
        }
        portal.layers().add(new Layer(name, kept, total, quadsIn, emitted, rejectedBy.clone()));
    }
