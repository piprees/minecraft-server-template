...
- **`portal.aura`**: `{"enabled": false}` to disable. Default is a derived bi-directional environmental spread (particles/sounds leak from the destination dimension around the portal in the source dimension). Disable for portals that should feel mundane rather than otherworldly.
