...
The mod itself ignores all of this at runtime — it only matters for seed rolling, which is how a hand-written config gets turned into an actual playable world with a chosen seed. Understanding this shapes how you should fill in `seedRoll` and `structures` so a roll can actually find and reward a good seed.
