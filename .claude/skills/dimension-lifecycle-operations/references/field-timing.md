...
| `structures.wants`/`shuns` (and the `seedRoll` forms they fall back to) | Read by `NoisePoolBuilder.build` when the pools are assembled, so the same runtime-rebuild timing as `structureDensity`: a want multiplies its structure's pool weight by 1.2 and a shun divides it by 1.5, on newly generated chunks only. Both are in `DimensionFingerprints.WORLDGEN_FIELDS` as `structureWants`/`structureShuns` (the RESOLVED id sets), so an edit WARNs at boot — but as the no-wipe variant of that warning, because the pool rebuilds from config. The seed bank re-keys too: `InputHash` hashes the whole config bar `seed`. |
...
| --- | --- |
| `borders.generation` | **Nothing in the mod applies it to a world.** `WorldBorderManager`'s own source comment: "`borders.generation` is deliberately NOT applied here: it is metadata for tooling." Four readers: the `unmined-render` sidecar's `generation_radius()` clamp on the rendered map area, `scripts/idle-tasks.sh`'s Chunky pre-generation extent (both falling back to `settings.json`'s `defaults.borders.generation`, then `PREGEN_BORDER_RADIUS`), `DimensionConfig.getLocateCap()` (generation + 1000, for the roller), and `DimensionLint.checkGenerationBorder`, which WARNs when it reaches more than the server's view distance past `borders.player`. Changing it needs no restart and no wipe — it takes effect on the renderer's next scheduled pass, or immediately via `./ops map render` — but it IS in the generation fingerprint, so it re-keys that dimension's seed bank. |
