...
| `environment.coordinateScale` | This is the **vanilla** dimension-type travel scale (nether-portal maths, map rendering), not the mod's own `portal.scale`. It is baked into the registered `DimensionType` the same way heights are; treat it as creation-time for the same reason. |

**A subtlety worth knowing but not relying on**: the `DimensionType` registry entry itself (built by `DimensionTypeBuilder.typeEntryFor`) *is* rebuilt fresh from config on every boot — the guard that returns an existing entry (`registry.getEntry(key)`) is an idempotency check within a single boot, not a permanent freeze across boots. That means the *non-storage-shape* `environment` fields below genuinely do take a fresh value each boot. It's only `minY`/`height`/`logicalHeight` (storage shape) and `coordinateScale` (bundled into the same object, and semantically a creation-time decision like `portal.scale`) that you should treat as fixed.
