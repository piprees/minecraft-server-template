...
5. **`STACK_VERSION` pinning**: `v2` (a bare major) floats onto the latest `v2.x.y` — you get minor/patch updates automatically via `./dev update`/`./ops update`. Pin an exact `v2.1.3` for reproducibility; bump majors deliberately and read the platform's migration notes first.
