...
4. **Delete world, player, and regenerable data:**
   - `data/world/` — one directory holds all of it: the overworld, `DIM-1`/`DIM1` (nether/end), `dimensions/<ns>/<slug>/` (every custom dimension, including `paradise_lost`), `playerdata/`, `stats/`, `advancements/`, `modplayerdata/`, `essentialcommands/`, `ec_player_profiles/`, and `data/world/data/` (`openpartiesandclaims` claims, `DistantHorizons.sqlite`(+`-shm`/`-wal`), `poi/`, `ledger.sqlite`, `level.dat`). Confirmed against a live server's directory layout (2026-08-15) — none of `world_the_nether/`, `world_the_end/`, `dimensions/`, `playerdata/`, `DistantHorizons.sqlite`, `poi/` or `ledger.sqlite` exist at the top level of `data/`; they're all nested under `data/world/`, so the single recursive delete covers them.
