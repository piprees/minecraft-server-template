...
4. **Delete world, player, and regenerable data:**
   - `data/world/` — one directory holds all of it: the overworld, `DIM-1`/`DIM1` (nether/end), `dimensions/<ns>/<slug>/` (every custom dimension, including `paradise_lost`), `playerdata/`, `stats/`, `advancements/`, `modplayerdata/`, `essentialcommands/`, `ec_player_profiles/`, and `data/world/data/` (`openpartiesandclaims` claims, `DistantHorizons.sqlite`(+`-shm`/`-wal`), `poi/`, `ledger.sqlite`, `level.dat`). None of `world_the_nether/`, `world_the_end/`, `dimensions/`, `playerdata/`, `DistantHorizons.sqlite`, `poi/` or `ledger.sqlite` exist at the top level of `data/` — they all nest under `data/world/`, so the single recursive delete covers them.
   - `data/unmined-web/maps/`, `data/unmined-web/index.html`, and `data/unmined-web/manifest.json` — rendered map tiles and the dimension/spawn manifest the web shell reads (all three re-generated on the next render pass; `manifest.json` is a separate file from `index.html` and lists the old world's dimensions/spawns until overwritten)
   ...
   - `data/dynamic-data-pack-cache/` — regenerable cache
   - `data/.dimension-setup/` — deploy.sh's per-dimension "already loaded once" markers (`SETUP_MARKERS_DIR`). Left in place, these make deploy.sh believe every dimension on the FRESH world was already set up on the OLD one and skip the `dimension load` call for all of them on the next deploy.
5. **Update the seed** in `.env` (local, backed up first to `.env.bak.<timestamp>`) and on the droplet (same backup pattern) — unless `--same-seed`, which keeps the current value. This updates `.env` only; it reaches terrain solely when the overworld's dimension config has `"seed": "env"`. Put the real seed in `config/custom-dimensions/dimensions/overworld.json` (or the overlay) and deploy it BEFORE running the reset ([TROUBLESHOOTING.md#t31](../../../../TROUBLESHOOTING.md#t31)).
...
7. **Restart via `deploy.sh --pull --non-interactive`** — this re-applies deploy.sh's normal post-boot configuration (world borders, game rules, LuckPerms permissions, spawn coordinates — config-driven when the overworld's dimension config has one chosen, `SPAWN_X/Y/Z` only as fallback), the same as any full deploy.
