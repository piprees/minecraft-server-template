...
**`RESTIC_PASSWORD` cannot be recovered if it's lost.** Every backup — every snapshot, in every retention tier — is unreadable without it. It lives in 1Password. Read this before touching anything else in this skill: if you are ever asked to rotate or regenerate it casually, stop and say so, per `docs/credentials.md § Rotation quick reference` ("Avoid rotating — existing backups are unreadable without the original. Start a new restic repo if you must").
