...
7. **`structures.force` overrides the biome check — but `/locate` still will not find an out-of-biome one.** Generation ignores the structure's biome list at a forced position (`ChunkGeneratorForcedBiomeMixin` replaces the predicate for those start attempts only), so an overworld structure forced into a nether dimension does generate. `/locate` reads a separate index that vanilla builds only for structures whose valid biomes intersect the dimension's biome source, and that index is untouched. Verify a forced placement with `/customdim structure-census` and the boot log, never with locate.
