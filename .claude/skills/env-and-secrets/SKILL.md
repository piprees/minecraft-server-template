...
4. **`DEPLOY_SSH_KEY` is not in `.env` at all.** It's read straight from the private key file at `DEPLOY_KEY_PATH` (default `~/.ssh/${BRAND_SLUG}_mc_deploy_key`) and pushed as a secret from its file contents. Looking for it in `.env` or `.env.example` wastes time — it isn't there by design.
