...
12. **Wait for RCON healthcheck** — up to 600s (many-dimension boots regularly exceed 5 minutes), polling every 10s, re-asserting autopause suspension each poll in case mc crash-restarted mid-boot. On timeout: `exit 1`, with the EXIT trap restoring the whitelist on the way out.
13. **Start anything compose's dependency wait gave up on** — a second `up -d --remove-orphans`, tolerant of failure.
14. **World borders + game rules + dimensions + permissions + spawn** — world borders are mod-owned (`custom-dimensions`' `WorldBorderManager`, not RCON) since v4 Phase 3, so this section activates the Nether/End/Paradise Lost dimensions for Chunky and runs one-time setup for any custom dimension not yet marked done in `data/.dimension-setup/` (gated so it only fires once per dimension, ever). Activation forceloads chunk [0,0], waits for the region file, then removes the ticket — **one dimension at a time**, and only where region files are absent. Three at once wedges the main thread permanently ([TROUBLESHOOTING.md#k6](../../../../TROUBLESHOOTING.md#k6)). Both loops exit 1 rather than continuing against a server that has stopped answering: `ACTIVATION_STALL_LIMIT` (3) unanswered flushes, `DIMENSION_FAILURE_LIMIT` (3) consecutive dimension-load failures. Scattered single failures still only defer that dimension to the next deploy. Restores real game rules (spawning/ticking/cycles back on — this is also where quiet-boot mode from section 4 ends), re-enables DH distant generation, reloads ServerCore + datapacks, runs `setup-permissions.sh`, and sets world spawn from `.env` **only if** the dimension config has no config-driven spawn (config wins over env if both are present).
