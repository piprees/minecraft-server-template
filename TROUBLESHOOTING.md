...
`execute in adventure:<slug> run seed` answers `Unknown dimension` on a healthy server with the config plainly present. `CreateWorldsMixin` filters vanilla's `createWorlds` loop down to the base worlds; the ~80 custom dimensions are built by `DimensionManager.getOrCreateDimension` when a player first enters one, because Distant Horizons and c2me build per-level state from `ServerWorldEvents.LOAD` and paying that per dimension per boot is what this avoids. Load it with `rcon-cli "customdim load <bare_slug>"` and poll — creation is queued to `END_SERVER_TICK`, so it lands a tick or two later, not immediately. The four base worlds are exempt: vanilla asks for them by key from paths with no lazy-creation hook.
